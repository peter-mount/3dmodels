...
module USB1() {
	difference() {
		...
		translate([6,0,0]) cube([10,10,5], center=true);
	}
}
/*
 * A 2 port USB connector as present on the Raspberry PI Models B/B+/2B & 3B
 *
 * Translate to the middle of the component before calling.
 *
...
/*
 * The CSI/DSI connector on the Compute board
 */
module ComputeCSI() {
	cube([16,3.74,1.5],center=true);
}
...
module MicroHDMI() {
	cube([11.2,8.4,3.2],center=true);
}

module MicroSD() {
	difference() {
		cube([12,11.5,2],center=true);
		translate([1.5,5.5,-0.5]) cube([7.2,1,4],center=true);
	}
}
